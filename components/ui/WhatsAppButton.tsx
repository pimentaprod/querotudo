...
// Páginas onde o botão flutuante é desnecessário (já têm CTA de WhatsApp)
const OCULTAR_EM = ['/checkout'];

export default function WhatsAppButton() {
  const pathname = usePathname();

  if (!WA_NUMERO) return null;
  ...
  return (
    ...
    >
      {/* Tooltip */}
      <span className="pointer-events-none hidden sm:block opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 transition-all duration-200 bg-gray-900 text-white text-xs font-semibold px-3 py-1.5 rounded-lg whitespace-nowrap shadow-lg">
        Falar com a loja
      </span>
